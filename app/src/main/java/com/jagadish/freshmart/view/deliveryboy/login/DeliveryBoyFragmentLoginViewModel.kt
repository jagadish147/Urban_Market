...
@HiltViewModel
class DeliveryBoyFragmentLoginViewModel @Inject
constructor(private val dataRepositoryRepository: DataRepositorySource) : BaseViewModel()  {
    /**
     * Data --> LiveData, Exposed as LiveData, Locally in viewModel as MutableLiveData
     */
    @VisibleForTesting(otherwise = VisibleForTesting.PRIVATE)
    val recipesLiveDataPrivate = MutableLiveData<Resource<RequestOtpRes>>()
    val recipesLiveData: LiveData<Resource<RequestOtpRes>> get() = recipesLiveDataPrivate
    ...
    fun requestOtp(cartReq: RequestOtpReq) {
        viewModelScope.launch {
            recipesLiveDataPrivate.value = Resource.Loading()
            wrapEspressoIdlingResource {
                dataRepositoryRepository.requestOTP(cartReq).collect {
    ...
    fun requestDeliveryBoyLogin(cartReq: RequestOtpReq) {
        viewModelScope.launch {
            recipesLiveDataPrivate.value = Resource.Loading()
            wrapEspressoIdlingResource {
                dataRepositoryRepository.requestDeliveryBoyLogin(cartReq).collect {
                    recipesLiveDataPrivate.value = it
                }
            }
        }
